...
async function reactivateProduct(user,rl,productsMenu,internalSystemMenu) {
    
    console.clear();
    console.log("📦 ============ REATIVAR PRODUTO ============ 📦\n");

    const sqlProducts = /* Cria a query para listar todos os produtos desativados juntamente com sua categoria e fornecedor. */
    `SELECT 
     p.id,
    ...
    const selectProduct = Number(await rl.question("\n📌 - Selecione o ID do produto que deseja reativar: ")); /* "selectProduct" recebe o ID do produto escolhido. */
    ...
    const sqlReactivateProduct = /* Cria a query para ativar o produto. */
    `UPDATE products
     SET active = 1
    WHERE id = ?;`

    await connection.execute(sqlReactivateProduct,[selectProduct]); /* Executa a ativação do produto no banco de dados. */
